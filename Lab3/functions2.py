#Все функции были определены, но не одна из них не вызвана ниже
#это сделана для того чтобы было легко проверить вызовом каждый функции по отдельности


def is_high_score(movie_name):

    for movie in movies:
        if movie['name'] == movie_name:
            return movie['imdb'] > 5.5
    return False


def get_high_score_movies(movies):
    high_score_movies = [movie for movie in movies if movie['imdb'] > 5.5]
    return high_score_movies


def get_movies_by_category(movies, category_name):
    category_movies = [movie for movie in movies if movie['category'] == category_name]
    return category_movies


def average_imdb_score(movies):
    if not movies:
        return 0.0

    total_score = sum(movie['imdb'] for movie in movies)
    average_score = total_score / len(movies)
    return average_score


def average_imdb_score(movies):
    if not movies:
        return 0.0

    total_score = sum(movie['imdb'] for movie in movies)
    average_score = total_score / len(movies)
    return average_score


movies = [
{
"name": "Usual Suspects",
"imdb": 7.0,
"category": "Thriller"
},
{
"name": "Hitman",
"imdb": 6.3,
"category": "Action"
},
{
"name": "Dark Knight",
"imdb": 9.0,
"category": "Adventure"
},
{
"name": "The Help",
"imdb": 8.0,
"category": "Drama"
},
{
"name": "The Choice",
"imdb": 6.2,
"category": "Romance"
},
{
"name": "Colonia",
"imdb": 7.4,
"category": "Romance"
},
{
"name": "Love",
"imdb": 6.0,
"category": "Romance"
},
{
"name": "Bride Wars",
"imdb": 5.4,
"category": "Romance"
},
{
"name": "AlphaJet",
"imdb": 3.2,
"category": "War"
},
{
"name": "Ringing Crime",
"imdb": 4.0,
"category": "Crime"
},
{
"name": "Joking muck",
"imdb": 7.2,
"category": "Comedy"
},
{
"name": "What is the name",
"imdb": 9.2,
"category": "Suspense"
},
{
"name": "Detective",
"imdb": 7.0,
"category": "Suspense"
},
{
"name": "Exam",
"imdb": 4.2,
"category": "Thriller"
},
{
"name": "We Two",
"imdb": 7.2,
"category": "Romance"
}
]


